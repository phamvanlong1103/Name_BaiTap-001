...
<div>
    <ValidationObserver v-slot="{ handleSubmit }">
        <form @submit.prevent="handleSubmit(onSubmit)">
            <ValidationProvider v-for="items in item" :key="items.id" :name="items.name" :rules="items.rules" v-slot="{ errors }">
                <div class="form-group">
                    <label>{{ items[items.key] }}</label>
                    <input v-if="!items.seclect" :type="items.type" class="form-control" v-model="items.model" />

                    <select v-if="items.seclect" class="form-control">
                        <option class="opt" v-for="item in items.options" :key="item" :value="item">
                            {{ item }}
                        </option>
                    </select>
